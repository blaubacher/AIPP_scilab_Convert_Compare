    ...
    for i=1:1:max(size(vars))
        ...
        if rows ~= []
            for j=1:1:max(size(rows))
                ...
                check_dots = strindex((json_txt(rows(j)),"."))
                if size(check_dots,"c") >1
                    dot_remove = check_dots($)
                    last_char = length(json_txt(rows(j)))
                    json_txt(rows(j)) = part(json_txt(rows(j))...
                    ,1:dot_remove-1) + part(json_txt(rows(j))...
                    ,dot_remove+1:last_char)
                end
            end
        end 
    end
